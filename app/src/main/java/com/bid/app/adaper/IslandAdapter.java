...
public class IslandAdapter extends BaseAdapter implements android.widget.SpinnerAdapter {

    private static final String TAG = IslandAdapter.class.getSimpleName();

    private Context mContext;
    private ArrayList<IslandInfo> islandInfos;

    public IslandAdapter(Context context, ArrayList<IslandInfo> centers) {
        this.islandInfos = centers;
        this.mContext = context;
    }

    @Override
    public int getCount() {
        return islandInfos.size();
    }


    @Override
    public Object getItem(int position) {
        return islandInfos.get(position);
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        textView.setText(islandInfos.get(position).getIsland_name());
        if (position == 0) {
